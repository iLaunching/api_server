"""
Migration: Add ai_acknowledge_text_color column to theme_configs
Adds text color property for AI acknowledgment indicator in appearance themes
"""

import asyncio
import asyncpg
import os
from dotenv import load_dotenv

load_dotenv()

async def add_ai_acknowledge_text_color():
    database_url = os.getenv("DATABASE_URL") or os.getenv("RAILWAY_DATABASE_URL")
    if not database_url:
        # Use Railway default if no env var
        database_url = "postgresql://postgres:TVzCDcmIDhjbquUUbrUMQExHEfXIwiNm@tramway.proxy.rlwy.net:12050/railway"
        print("⚠️  Using default Railway DATABASE_URL")
    
    conn = await asyncpg.connect(database_url)
    
    try:
        print("🔄 Adding ai_acknowledge_text_color column to theme_configs table...")
        
        # Step 1: Add the ai_acknowledge_text_color column
        await conn.execute("""
            ALTER TABLE theme_configs 
            ADD COLUMN IF NOT EXISTS ai_acknowledge_text_color VARCHAR(7);
        """)
        print("  ✓ Added ai_acknowledge_text_color column")
        
        # Step 2: Get the appearance option set ID
        appearance_set_id = await conn.fetchval(
            "SELECT id FROM option_sets WHERE name = 'appearance'"
        )
        
        if not appearance_set_id:
            print("  ⚠️  Appearance option set not found")
            return
        
        print(f"  ✓ Found appearance set ID: {appearance_set_id}")
        
        # Step 3: Update Sun theme
        sun_id = await conn.fetchval("""
            SELECT id FROM option_values 
            WHERE option_set_id = $1 AND value_name = 'sun'
        """, appearance_set_id)
        
        if sun_id:
            await conn.execute("""
                UPDATE theme_configs 
                SET ai_acknowledge_text_color = $1
                WHERE option_value_id = $2
            """, '#110f75', sun_id)
            print("  ✓ Updated Sun theme: ai_acknowledge_text_color=#110f75")
        
        # Step 4: Update Moon theme
        moon_id = await conn.fetchval("""
            SELECT id FROM option_values 
            WHERE option_set_id = $1 AND value_name = 'moon'
        """, appearance_set_id)
        
        if moon_id:
            await conn.execute("""
                UPDATE theme_configs 
                SET ai_acknowledge_text_color = $1
                WHERE option_value_id = $2
            """, '#ffffff', moon_id)
            print("  ✓ Updated Moon theme: ai_acknowledge_text_color=#ffffff")
        
        # Step 5: Update Space theme
        space_id = await conn.fetchval("""
            SELECT id FROM option_values 
            WHERE option_set_id = $1 AND value_name = 'space'
        """, appearance_set_id)
        
        if space_id:
            await conn.execute("""
                UPDATE theme_configs 
                SET ai_acknowledge_text_color = $1
                WHERE option_value_id = $2
            """, '#ffffff', space_id)
            print("  ✓ Updated Space theme: ai_acknowledge_text_color=#ffffff")
        
        # Step 6: Update Star theme
        star_id = await conn.fetchval("""
            SELECT id FROM option_values 
            WHERE option_set_id = $1 AND value_name = 'star'
        """, appearance_set_id)
        
        if star_id:
            await conn.execute("""
                UPDATE theme_configs 
                SET ai_acknowledge_text_color = $1
                WHERE option_value_id = $2
            """, '#ffffff', star_id)
            print("  ✓ Updated Star theme: ai_acknowledge_text_color=#ffffff")
        
        print("\n✅ Migration completed successfully!")
        print("📋 Summary:")
        print("   - Sun: ai_acknowledge_text_color=#110f75 (dark blue)")
        print("   - Moon: ai_acknowledge_text_color=#ffffff (white)")
        print("   - Space: ai_acknowledge_text_color=#ffffff (white)")
        print("   - Star: ai_acknowledge_text_color=#ffffff (white)")
        
    except Exception as e:
        print(f"\n❌ Migration failed: {e}")
        raise
    finally:
        await conn.close()

if __name__ == "__main__":
    asyncio.run(add_ai_acknowledge_text_color())
